...
class Message extends Base {
  ...
  async downloadMediaMessage() {
    let buff = await this.data.download();
...
//Fixed Amarok
  async sendMessage(
    content,
    opt = { packname: "Kakashi-Md", author: "TurboMods" },
    type = "text"
  ) {
...
async groupInfo(jid) {
 await this.client.groupMetadata(jid)
 }
  /**
   *
   * @param {string} jid
...
async revokeLink(jid){
return await this.client.groupRevokeInvite(jid)
 }
...
async inviteLink(jid) {
 await this.client.groupInviteCode(jid);
 }
